...
function getInlineMimeType(inline: GeminiPart['inlineData'] | GeminiPart['inline_data']) {
  if (!inline) return 'image/png';
  if ('mimeType' in inline) return inline.mimeType || 'image/png';
  return (inline as { mime_type?: string }).mime_type || 'image/png';
}
...
export async function callGeminiImage({ apiKey, imageDataUrl, stylePrompt, phrase }: GenerateRequest) {
  ...
  const outputMimeType = getInlineMimeType(inline);
